...
client.on('messageCreate', async message => {
    ...
    if (message.guild) {
        ...
        if (message.content === '!criarcategoria') {
            ...
            const embed = new EmbedBuilder().setTitle('📝 Prova de Recrutamento 24h - BMRP').setDescription('Clique no botão abaixo para iniciar o teste nas suas DMs.').setColor('#2b2d31');
    ...
    // RESPOSTAS DAS PROVAS VIA DM (MENSAGENS PRIVADAS)
    const test = activeTests.get(message.author.id);
    if (!test) return;

    const content = message.content.trim();
    await message.delete().catch(() => {});
    if (test.lastBotMessage) await test.lastBotMessage.delete().catch(() => {});

    if (test.step === 0) {
        if (test.subStep === 'nome') {
            test.data.nome = content;
            test.subStep = 'nick';
            test.lastBotMessage = await message.channel.send('🎮 Qual é o seu **Nick no Jogo**?');
            return;
        }
        if (test.subStep === 'nick') {
            test.data.nick = content;
            test.subStep = 'level';
            test.lastBotMessage = await message.channel.send('⭐ Qual é o seu **Level no Jogo**?');
            return;
        }
        if (test.subStep === 'level') {
            test.data.level = content;
            test.subStep = 'tempo';
            test.lastBotMessage = await message.channel.send('⏳ Há **quanto tempo você joga no servidor**?');
            return;
        }
        if (test.subStep === 'tempo') {
            test.data.tempo = content;
            test.step = 1; 
            const totalQ = test.questions.length;
            const firstQ = test.questions[0];
            test.lastBotMessage = await message.channel.send(`✅ Cadastro concluído!\n\nIniciando a **Prova de Regras (${totalQ} Questões)**.\nResponda com o número da alternativa (\`1\`, \`2\` ou \`3\`):\n\n**Pergunta 1/${totalQ}:**\n${firstQ.question}`);
            return;
        }
    }

    const totalQuestions = test.questions.length;
    const currentQ = test.questions[test.step - 1];

    if (!['1', '2', '3'].includes(content)) {
        const warning = await message.channel.send('⚠️ Responda digitando apenas o número da alternativa (1, 2 ou 3).');
        setTimeout(() => warning.delete().catch(() => {}), 4000);
        return;
    }

    if (content === currentQ.correct) test.score++;
    test.step++;

    if (test.step - 1 < totalQuestions) {
        const nextQ = test.questions[test.step - 1];
        test.lastBotMessage = await message.channel.send(`**Pergunta ${test.step}/${totalQuestions}:**\n${nextQ.question}`);
    } else {
        ...
        if (test.score > 5) {
            status = '✅ **APROVADO**';
            await message.channel.send(`${status} com **${test.score}/${totalQ}** acertos!\nEntre no grupo do WhatsApp:\n${WHATSAPP_LINK}`);
        } else if (test.score === 5) {
            status = '⚠️ **APROVADO COM OBSERVAÇÃO**';
            color = '#FFFF00';
            await message.channel.send(`${status} com **${test.score}/${totalQ}** acertos! Você passou na metade da prova.\nEntre no grupo do WhatsApp:\n${WHATSAPP_LINK}`);
        } else {
            status = '❌ **REPROVADO**';
            color = '#FF0000';
            cooldowns.set(message.author.id, Date.now() + (5 * 60 * 60 * 1000));
            await message.channel.send(`${status}. Acertou apenas **${test.score}/${totalQ}**.\nTente novamente em **5 horas**.`);
        }
        ...
        if (logChannel) {
            const embedLog = new EmbedBuilder()
                ...
                .setColor(color)
                .addFields(
                    { name: '👤 Discord', value: `${message.author}`, inline: false },
        ...
        activeTests.delete(message.author.id);
    }
});
    ...
    if (interaction.isButton()) {
        if (interaction.customId === 'start_quiz') {
            ...
            if (activeTests.has(userId)) {
                return interaction.reply({ content: '⚠️ Você já possui um processo de recrutamento em andamento nas DMs!', ephemeral: true });
            }
            ...
            try {
                ...
                activeTests.set(userId, { step: 0, subStep: 'nome', data: {}, score: 0, questions: shuffledQuestions, guild: interaction.guild, lastBotMessage: null });
                await interaction.reply({ content: '✅ Verifique suas **Mensagens Diretas (DMs)**!', ephemeral: true });
                
                const initialMsg = await dmChannel.send('📋 **Cadastro de Recrutamento BMRP**\nPara começarmos, qual é o seu **Nome**?');
                activeTests.get(userId).lastBotMessage = initialMsg;
            } catch (error) {
                await interaction.reply({ content: '❌ Não consegui te enviar mensagem privada. Abra suas DMs!', ephemeral: true });
            }
        }

        if (interaction.customId === 'order_drugs' || interaction.customId === 'order_weapons' || interaction.customId === 'order_explosives') {
            const modalMap = {
...
// LOGIN SEGURO VIA VARIÁVEL DE AMBIENTE
